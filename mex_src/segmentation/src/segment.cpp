...
void Histogram::extractForegroundHistogram(std::vector<cv::Mat> & imgChannels, cv::Mat weights, bool useMatWeights, int x1, int y1, int x2, int y2, cv::Mat mask_img, bool mask_flag)
{
    ...
    for (int y = y1; y < y2+1; ++y)
    {
        ...
        for (int x = x1; x < x2+1; ++x)
        {
            ...
            for (int dim = 0; dim < m_numDim; ++dim)
            {
                if(mask_flag)
                {
//                     mexPrintf("%i", (int)mask_img.at<uchar>(y,x)); 
                    if((int)mask_img.at<uchar>(y,x) > 0) // I'm not sure exactly how data types are converted from Matlab to C++ so make sure it works.
                    {    
