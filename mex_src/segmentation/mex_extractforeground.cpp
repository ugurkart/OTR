...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[] ) {
    ...
    if(nrhs < 5) mexErrMsgTxt("Valid input: [image], [x1, y1, x2, y2], bins, [mask], mask_flag.");
    ...
    // Adding mask by Ugur
    // If mask_flag = true, use it. Else, do it as in original code
    cv::Mat mask_img = toMat(prhs[3], CV_8U, true);   
    
    bool mask_flag = false;
    int flag = static_cast<int>(mxGetScalar(prhs[4]));
//     mexPrintf("Flag: %d", flag);
    if(flag == 1)
    {
           mask_flag = true;
//         mexPrintf("Flag true\n");
